...
export default function TrackHint({ visible, driverPanelOpen = false }: Props) {
  const [dismissed, setDismissed] = useState(false);

  // Outer div owns positioning + slide transition; inner motion.div owns opacity.
  // right: calc(100% - 438px) places the left edge at 228px (220px grid + 8px gap).
  return (
    <div
      className="pointer-events-none"
      style={{
        position: 'absolute',
        right: driverPanelOpen ? 'calc(100% - 438px)' : '12px',
        bottom: '12px',
        width: 210,
        zIndex: 20,
